...
if($qualify -match "Join-Path \$root 'logs'"){
  throw '[FAIL] qualification logs are still written inside package root'
}
...
Need $verify '\(\?:logs\|evidence\|results\|test-results' 'package hardcoded-profile scan excludes transient runtime directories'
